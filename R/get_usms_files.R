...
#' @param redelac logical FALSE to parse the file instead to use XML
#' only if usms_list is empty
...
get_usms_files <- function(workspace,
  ...
  if (lifecycle::is_present(file_name)) {
    ...
    file_name <-
      usms_file # to remove when we update inside the function
  }
  ...
  # Getting usms_list
  usms_full_list <- find_usms_soils_names(xml_doc = xml_doc,
                                          xml_name = "usm")
  ...
  for (i in 1:usms_nb) {
    if (redelac && (is.null(usms_list)) {
      ...
      usm_name <-
        str_extract(firstLine, pattern = "(?<=<usm nom=\").*(?=\">)")

      txtToWork <-
        usms_txt_list_only[2 + ((i - 1) * 24):(21 + ((i - 1) * 24))]
      txtOfInterest <- txtToWork %>% str_subset(pattern = "<f.*</f")
      usm_files <- txtOfInterest %>%
        str_extract(pattern = "(?<=>).*(?=</f)") %>%
        str_subset(pattern = "null", negate = TRUE)
    } else {
      ...
      usm_files <-
        unique(usm_files[-grep("\\.obs|\\.lai|null", usm_files)])
    }
    ...
    if (check_plt) {
      ...
      plt_files_path <-
        vector(mode = "character", length(plt_files))
    ...
    if ("sols" %in% file_type) {
      sols_file_path <- suppressWarnings(normalizePath(file.path(workspace_path, "sols.xml")))
    ...
    if ("pargen" %in% file_type) {
      pargen_file_path <- suppressWarnings(normalizePath(file.path(workspace_path, "param_gen.xml")))

      if (!file.exists(pargen_file_path))
        pargen_file_path <- suppressWarnings(normalizePath(file.path(
          javastics, "config", "param_gen.xml"
        )))
    ...
    if ("parnew" %in% file_type) {
      parnew_file_path <- suppressWarnings(normalizePath(file.path(
        workspace_path, "param_newform.xml"
      )))

      if (!file.exists(parnew_file_path))
        parnew_file_path <- suppressWarnings(normalizePath(file.path(
          javastics, "config", "param_newform.xml"
        )))
