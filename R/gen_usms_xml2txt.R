...
gen_usms_xml2txt <- function(javastics = NULL,
  ...
  if (lifecycle::is_present(javastics_path)) {
    ...
    javastics_path <-
      javastics # to remove when we update inside the function
  }
  # workspace_path
  if (lifecycle::is_present(workspace_path)) {
    ...
    workspace_path <-
      workspace # to remove when we update inside the function
  }
  ...
  if (lifecycle::is_present(target_path)) {
    ...
    target_path <-
      out_dir # to remove when we update inside the function
  }
  # usms_list
  ...
  if (is.null(usms_file))
    usms_file_path <- file.path(workspace_path, "usms.xml")
  else
    usms_file_path <- file.path(workspace_path, usms_file)
  ...
  dominance <-
    get_param_value(usms_doc, param_name = "dominance")$dominance

  nbplantes <-
    get_param_value(usms_doc, param_name = "nbplantes")$nbplantes


  flai_usms <-
    vector(mode = "list", length = length(full_usms_list))
  names(flai_usms) <- full_usms_list
  usm_index <-
    1 # This is equivalent of i, but tracks which usm we are doing in
  ...
  for (i in seq_along(lai_file_path)) {
    if (dominance[i] == 1) {
      ...
      flai_usms[[usm_index]] <-
        c(flai_usms[[usm_index]], lai_file_path[i])
    }
  ...
  all_files_exist <- unlist(lapply(all_files_list, function(x)
  ...
  out_files_java_path <-
    file.path(javastics_path, "config", out_files_def)
  ...
  if (!java_converter && redelac) {
    ...
    all_file_list_single <-
      unique(unlist(lapply(all_files_list, function(x) {
        x$paths
      })))
    ...
    all_file_list_single <-
      all_file_list_single[grep(pattern = "sols",
                                all_file_list_single,
                                invert = TRUE)]
  ...
  res <- foreach (
    ...
    if (java_converter) {
      ...
      for (f in seq_along(xml_files_path)) {
        ...
        if (grepl(pattern = "sols", x = file_path) && !redelac) {
        ...
        if (!redelac || !grepl(pattern = "sols", x = file_path)) {
          ...
          gen_files_status[f] <- file.copy(file.path(workspace,
                                                     paste0(usm_data$nomsol,
                                                            "_sol.txt")),
                                           file.path(usm_path, "param.sol"))
        }
      }
    ...
    to_copy_idx <-
      !file.exists(file.path(usm_path, basename(out_files_path)))
  ...
  if (!all(exec_status)) {
    warning("Errors have been detected for usm(s):",
